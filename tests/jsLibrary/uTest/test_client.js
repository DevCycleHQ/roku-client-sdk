...
describe('client tests', () => {
  ...
  it('test install', async () => {
    nock(baseUrl).post(`/${sessionId}/install`, requestInstall).reply(200, respWithEmptyValue)
  ...
  it('test set timeouts', async () => {
    nock(baseUrl).post(`/${sessionId}/timeouts`, requestTimeout).reply(200, respWithEmptyValue)
    const response = await client.setTimeouts('implicit', 1000)
    expect(JSON.stringify(respWithEmptyValue)).to.equal(JSON.stringify(response.data))
  })

  it('test get screen source', async () => {
    nock(baseUrl).get(`/${sessionId}/source`).reply(200, respWithSource)
  ...
  it('test deleteSession', async () => {
    nock(baseUrl).delete(`/${sessionId}`).reply(200, respWithEmptyValue)
